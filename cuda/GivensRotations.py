...
class RotMatFunction(torch.autograd.Function):
    ...
    def backward(ctx, lossGrad):
        ...
        lossGrad2, thetaGrad = rotMatcuda.backward(thetas.detach(), torch.clone(ux).detach(), torch.clone(lossGrad.detach().contiguous()))
        return lossGrad2, thetaGrad
...
class RotMat(torch.nn.Module):
    def __init__(self, N, M=None):
        ...
        self.thetas = torch.nn.Parameter(torch.zeros(getThetaCount(N,M)).to(device).fill_(np.pi * 4))
